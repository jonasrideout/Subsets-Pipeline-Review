...
export default function StatCard({ label, value, subValue, target, goalPct, pacePct, onClick, active }: StatCardProps) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 12, fontWeight: 500, color: "#8b90a0", marginBottom: hasMetrics || subValue ? 8 : 0 }}>
        {label}
      </div>
      {subValue && !hasMetrics && (
        <div style={{ fontSize: 11, fontWeight: 600, color: "#0a7a50", borderTop: "1px solid #f0f1f5", paddingTop: 6 }}>
          {subValue}
        </div>
      )}
      {hasMetrics && (
